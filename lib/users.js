...
export function readUsers(env = envUsers()) {
  ...
  if (!raw) {
    ...
    // `legacy` rides along so the app can say out loud that it's signing
    // everyone in under a placeholder name. Silently attributing the log to
    // "Owner" is worse than the log not existing — it looks like it works.
    return {
      users: [{ name: LEGACY_NAME, slug: slugify(LEGACY_NAME), pin, legacy: true }],
      error: null,
    };
  }
